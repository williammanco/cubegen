...
const gammaPattern = 'GAMMA=\\s*([0-9]*[.][0-9]*)';
...
function readPixelsRawRLE(buffer, data, offset, fileOffset, scanlineWidth, numScanlines) {
  ...
  function readPixelsRaw(buffer, data, offset, numpixels) {
    ...
    const numRead = readBufferOffset(data, offset, numExpected);
  }
...
function parseHDR(buffer) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if ((match = line.match(formatPattern))) {
      rle = true;
    } else if ((match = line.match(gammaPattern))) {
      gamma = Number(match[1]);
    ...
    Promise.all(
      jsonChunk.map(
        entry => new Promise((resolve, reject) => {
          ...
          resolve({
            name,
            file: parseHDR(new Uint8Array(binary)),
          });
        }),
      ),
    ).then((faces) => {
